...
- Install platform version via `composer require six-it/platform-version` and follow steps described
...
- Get skeleton via `composer create-project six-it/pimcore-skeleton my-project`.
- Install platform version via `composer require six-it/platform-version`. Eventually you need to adapt versions of other
  pimcore packages to apply to versions required by platform version. Please follow instructions of composer.
...
Use following command to install additional Pimcore modules to the project: `composer require six-it/<MODULE>`.
...
- Update to a new Pimcore Platform version:
    - if you want to update to a specific version, use the command `composer require six-it/platform-version:2026.1 --no-update` to update your `composer.json`. Afterwards run `composer update`.
    - if you want to update to the latest version (and the constraint in your `composer.json` allows further updates),
      use the command `composer update six-it/platform-version`.
...
Keep in mind that the `six-it/platform-version` repository does not specify required packages.  
It only specifies conflicting packages to ensure compatible versions are installed.
The command `composer require six-it/platform-version --update-with-all-dependencies` will therefore not work.
